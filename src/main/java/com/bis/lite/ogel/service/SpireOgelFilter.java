...
public class SpireOgelFilter {
    ...
    private static String ratingCode;
    ...
    public static List<SpireOgel> filterSpireOgels(List<SpireOgel> ogelsList, String rating,
        ...
        return ogelsList.stream().filter(
                ogel -> (applyRatingIsIncluded(ogel)) &&
                        categorites.contains(ogel.getCategory()) &&
                        (applyExcludedCountriesIfPresent(ogel)
                                || applyIncludedCountriesIfPresent(ogel)))
    ...
    private static final boolean applyRatingIsIncluded(SpireOgel ogel) {
        return ogel.getOgelConditions().stream().map(OgelCondition::getRatingList).flatMap(l -> l.stream())
                .anyMatch(r -> r.getRatingCode().equalsIgnoreCase(ratingCode));
    }

    private static final boolean applyExcludedCountriesIfPresent(SpireOgel ogel) {
    ...
    private static final boolean applyIncludedCountriesIfPresent(SpireOgel ogel) {
